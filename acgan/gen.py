...
parser.add_argument('--label', type=int, default=5, help='Number of classes for AC-GAN')
...
#eval_label = np.full((opt.batchSize),opt.label)
print("eval_label:",eval_label)
...
for epoch in range(opt.niter):
    for i, data in enumerate(dataloader, 0):
        ...
        curr_iter = opt.ntier * len(dataloader) + i
...
vutils.save_image(real_cpu, '%s/real_samples_%s.png' % (opt.outf, opt.label ))

print('Label for eval = {}'.format(eval_label))
fake = netG(eval_noise)
vutils.save_image(fake.data,'%s/fake_samples_%s.png' % (opt.outf, opt.label))
